...
def solve(graph, num_bus, size_bus, constraints):
    ...
    while nodes_left and bus_index < num_bus:
        ...
        # Add one extra left over person to each bus
        if left_over > 0:
            if len(output[bus_index]) > num_people_per_bus:
...
def find_node_max_neighbors(nodes, adj_list, bus, rowdy_dict, nodes_used):
    ...
    if len(bus) == 0:
        return max(nodes, key=lambda n: len(__intersection(adj_list[n], nodes)))
...
def main_on_specific_input(size, input_f):
    ...
    if not os.path.isdir(path_to_debug_outputs):
        os.mkdir(path_to_debug_outputs)
...
if __name__ == '__main__':
    #main_on_specific_input("medium", "166")
